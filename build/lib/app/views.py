...
def index():
    ...
    get_service = MassageService.query.all()
    return render_template('index.html', get_service=get_service)
...
def booking():
    ...
    # Beállítjuk az alapértelmezett dátumot (GET vagy üres POST esetén)
    ...
    # Generáljuk az összes félórás időpontot 08:00 - 20:00 között
    all_times = [
    ...
    # Lekérjük az összes foglalást a kiválasztott napra
    bookings = Booking.query.filter_by(date=selected_date).all()

    # Blokkolt időpontok számítása (±30 perc minden foglalás körül)
    blocked_set = set()
    for b in bookings:
    ...
    # Stringes formátum a template-hez
    ...
    # Ha a form érvényes és POST-tal jött
    if form.validate_on_submit():
...
def edit_booking(booking_id):
    ...
    # Generáljuk az összes félórás időpontot
    all_times = [
        (datetime.combine(
    ...
    # Foglalt időpontok kizárása (kivéve a saját foglalásunkat)
...
def profile():
    ...
    if form.validate_on_submit():
        ...
        # Ha jelszó helyes: frissíthet
        current_user.name = form.name.data
        current_user.email = form.email.data

        # Ha megadott új jelszót, frissítjük
...
def admin():
    ...
    all_get_service = MassageService.query.all()
...
def services():
    """Szolgáltatások oldala, ahol az elérhető szolgáltatások
        listázva vannak."""
    all_get_service = MassageService.query.all()
    return render_template('services.html', get_service=all_get_service)
...
def free_slots():
    ...
    for day in upcoming_days:
        if day.weekday() in [5, 6]:  # Hétvége kihagyása
            continue

        bookings = Booking.query.filter_by(date=day).all()

        # Blokkolt idők HH:MM formátumban (±30 perces blokkolással)
        blocked_times = set()
